...
Read in logger data
(Example data here is wood mouse logger detection data from Raulo et al. 2021, ISMEJ)
```{r}
...
A) Depending on the temporal resolution of your observation, decide if you want to aggregate your data stream within a time frame. Here for example, we are aggregating loggerdata so that it is treating all logging records of individual X in location Y within one hour as one single observation.
```{r}
...
The "units" of this function are night-location combinations, where two individuals are declared either "associated" or "not associated" depending on the sliding window criterion of what counts as "associated"

This index calculates the proportion of night-location combination where Individual A and Individual B where observed within x amount of time from each other in the same place relative to the number of night-location combinations when they were observed in general (either just one or the other or both either associated or not) during the overlap of their lifespans.

- As input, this takes a gbi-matrix, t list of times and time-location combinations, and these are all separately defined in the next section (2B)

- The possible association indices to use are:
SRI: Simple Ratio index
HW: Half-weight index 

SRI_successes: Nominator of SRI - Sum of the number of instance (logger-night combinations) tow individuals were observed associated

SRI_fails: Sum of the number of instance (logger-night combinations) tow individuals were observed NOT associated
...
- GBI (Group by Individual) is a traditionally a matrix where rows are "groups" and columns are all individuals and numbers in the matrix depict how many times this individual was observed in this group. Here, the rows ("groups") are defined as one pair of individuals per specific night-location combination. So for example one row ("group") in the gbi matrix could be Susan-Nathalie-Location1-Night1, and it would only have binary input numbers (1/0) for columns "Susan" and "Nathalie". 1 for all the nights-location combos when they were observed "associated" , 0 for all the night-location combos they were not.

Here, whether a pair is considered "associated" during a specific night-logger combination, depends on the Sliding window intimacy criterion set by the user. In other words, whether a pair gets a 0 or a 1 for a certain row ("logger-night-group") depends on whether their observation record satisfies the sliding window criteria of how close in time to each other they need to have been observed in the same location to be considered "associated".
...
## (Note: while the code seems to filter instances based on them happening during the same night, this is actually getting rid of instances within same night-logger combination, because instances are already, by definition (see line 273) happening within the same logger.)
...
Second part for finding all instances where individual was observed alone within logger-night combination

Make one-individual-per-group gbi addition, remember to add all "groups" in double, to mimic the structure of the existing gbi, as that has tow inputs for each pair (one for indovidial 1 and one for individual 2 of each pair). 
...
#now  find the instances that we see only one individual within x time within night_logger combination
for (i in 1:length(ids)){
add.to<-nrow(GBI2)+1
